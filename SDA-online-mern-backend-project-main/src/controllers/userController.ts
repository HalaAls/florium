...
export const processRegisterUser = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (isUserExists) {
      ...
      const error = createHttpError(409, `User already exist with the email ${email}`)
      throw error
    }
...
export const updateUser = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // Check if the updated email already exists in the database (excluding the current user being updated)
...
export const banUser = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const email = req.params.email
    const user = await userService.updateBanStatus(email, true)
    res.status(200).json({ message: 'Banned the User successfully', payload: user })
  } catch (error) {
    next(error)
  }
}
...
export const unBanUser = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const email = req.params.email
    const user = await userService.updateBanStatus(email, false)
    res.status(200).json({ message: 'Unbanned the User successfully', payload: user })
  } catch (error) {
    next(error)
  }
}
...
export const grantRole = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!userExist) {
      throw createHttpError(404, `User Not Found With This Email ${email} `)
    }
    userExist.isAdmin = !isAdmin
    const user = await userExist.save()
    res.status(200).json({ message: 'Granted Role successfully', payload: user })
  } catch (error) {
    next(error)
  }
}
