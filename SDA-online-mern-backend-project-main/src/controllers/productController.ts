...
export const createProduct = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    console.log('backend image is ', image)
    ...
    const products = await productService.createNewProduct(image, productData)

    res.status(201).send({ message: 'product is created', payload: products })
  } catch (error) {
    next(error)
  }
}
